...
A full example Java project, showing how to drive the WASM module end to end, lives in a separate
repository: [slang-wasm-endive](https://github.com/RefuX/slang-wasm-endive). The Java snippets in
this README are taken from it to illustrate the API.
...
| Java                                                         | 11+     | Running the [example Java consumer](https://github.com/RefuX/slang-wasm-endive) and its tests (optional — only the `.wasm` itself is required to use this library from another runtime) |

Only the `.wasm` artifact is built from this repository. The example Java consumer (with its Gradle
wrapper and tests) lives in a separate repository — see
[slang-wasm-endive](https://github.com/RefuX/slang-wasm-endive).
...
At configure time CMake checks whether `include/slang.h` is newer than the generated file (or
whether the generated file exists at all — it is gitignored, not committed). If either is true, it
automatically runs the [enum binding generator](#enum-binding-generator) to (re)create the C++
metadata blob. The Java enum sources are _not_ regenerated by the build — they are opt-in (see
below).
...
None of these are wired into the `wasi`/`slang-wasm-wasi` presets by default, since they all trade
...
showing the pattern end to end. It drives the module via
...
To try it, clone [slang-wasm-endive](https://github.com/RefuX/slang-wasm-endive) and add its Gradle
project to your build, or copy the sources. The snippets below are drawn from that project.
...
try (var slang = SlangCompiler.forSpirvFromWasm(Path.of("slang-wasm-wasi.wasm"))) {
...
try (var slang = SlangCompiler.builder()
        .wasm(Path.of("slang-wasm-wasi.wasm"))
...
example consumer. They double as a regression suite for the `.wasm` artifact itself, and skip
automatically when the artifact has not been built yet.
...
The C++ file is not committed — CMake generates it automatically at configure time, either because
it doesn't exist yet or because `slang.h` is newer than it. The Java enums are **not** generated by
default — pass `--java-out` to opt in, and `--java-package` to control the package they are
declared in (default `org.shaderslang.wasm.enums`).

### Running the generator manually

```bash
# From the repository root — C++ metadata only (the default):
python3 source/slang-wasm-wasi/tools/generate-slang-bindings.py

# With explicit paths:
python3 source/slang-wasm-wasi/tools/generate-slang-bindings.py \
    --slang-h  include/slang.h \
    --cpp-out  source/slang-wasm-wasi/slang-wasm-enum-metadata.cpp

# Also generate the Java enums into a chosen directory and package:
python3 source/slang-wasm-wasi/tools/generate-slang-bindings.py \
    --java-out     path/to/java/org/example/enums \
    --java-package org.example.enums
```
...
The [slang-wasm-endive](https://github.com/RefuX/slang-wasm-endive) example layers a Java
builder/typed-request API with generated enums throughout on top of these exports.
